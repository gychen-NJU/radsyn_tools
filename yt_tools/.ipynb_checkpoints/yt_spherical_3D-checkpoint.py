...
class spherical_data():
    ...
    def add_field(self, f_names,  frame='all', dr=0.0, n_cut=0):
        chioce = self.ts if frame=='all' else self.ts[frame]
        begin  = 0 if frame=='all' else frame
        x_sample, y_sample, z_sample, bbox_cart, bbox = self.load_xyz()
        n = self.n_sph
        for idx, ds in enumerate(choice):
            idx+=begin
            ...
            if not check:
                ...
                region  = ds.r[::n*1j,::n*1j,n::n*1j]
                temp    = np.load(os.path.join(self.folder,'index.npz'))
                indexs  = temp['indexs']
