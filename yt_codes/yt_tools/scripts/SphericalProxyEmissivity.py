...
def magline_stepper(rtp, **kwargs):
    ...
    if np.any(np.isnan(rtp)) or np.any(np.isinf(rtp)):
        return np.full(3,np.nan)
    r,t,p = rtp if rtp.ndim==1 else rtp.T
    t = t % (np.pi)
    p = p % (plist.max())
    if rtp.ndim==1:
        if r>rlist.max() or r<rlist.min() or np.any(np.isnan(rtp)) or np.any(np.isinf(rtp)):
            return np.full(3,np.nan)
        rtp = np.array([r,t,p])
        br = br_interp(rtp)
        bt = bt_interp(rtp)
        ...
        if bn<eps:
            return np.full(3,np.nan)
        ret = np.array([br[0]/bn,bt[0]/bn/r,bp[0]/bn/(r*np.sin(t)+eps)])
        return ret
    else:
        br = br_interp(rtp)
        bt = bt_interp(rtp)
        ...
        bn[null_point]+=eps
        ret = np.stack([br/bn,bt/bn,bp/bn], axis=-1)/np.stack([np.ones_like(r),1/r,1/(r*np.sin(t)+eps)], axis=-1)
...
tlist  = rtp[1,0,:,0]
plist  = rtp[2,0,0,:]
device = info.get('device', 'cuda' if torch.cuda.is_available() else 'cpu')
Jrtp   = rot(Brtp, rtp=rtp, device=device)
