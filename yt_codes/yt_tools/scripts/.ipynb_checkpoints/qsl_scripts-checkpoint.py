...
def calculate_qsl_range(start_idx, end_idx, boundary_points, progress_list, total_tasks, lock, 
                        print_interval=100, t0=0, r_min=1.0, coords='spherical'):
    res_list = []
    for icnt in range(start_idx, end_idx):
        ...
        if np.max(lineP[9:12])<1e-8:
            ...
            lineF, lineB, lambda_F, lambda_B = instance.LineIntegrate(lineP, bxyz, Jacobi, dxyz, r_min=r_min, geometry=coords, max_step=max_step)
...
def parallel_qsl(boundary_points, n_cores=10, print_interval=100, r_min=1.0, coords='spherical'):
    ...
    with ProcessPoolExecutor(max_workers=n_cores) as executor:
        ...
        for i in range(n_cores):
            ...
            futures.append(executor.submit(calculate_qsl_range, start_idx, end_idx, boundary_points, progress_list, total_tasks, lock, print_interval, t0, r_min, coords))
...
parser.add_argument('-g', type=str  , help='geometry, `cartesian` or `spherical`'     , required=False, default='spherical'       )
parser.add_argument('--max_step', type=int  , help='maximum step for qsl intergral'   , required=False, default=100000            )
...
if coords=='spherical':
    ...
    results        = parallel_qsl(points, n_cores=n_cores, print_interval=print_interval, r_min=r_min, coords=coords)
    np.save('./qsl_results.npy', np.array(results))
elif coords=='cartesian':
    bbox   = instance.bbox
    nxyz   = instance.dimensions*instance.sample_level
    dxyz   = (bbox[:,1]-bbox[:,0])/nxyz
    bxyz   = instance.return_bxyz()
    # Jacobi = geometry.jacobi_matrix(bxyz, dxyz).detach().cpu().numpy()
    bmag           = np.linalg.norm(bxyz,axis=0)[np.newaxis,:,:,:]
    bhat           = bxyz/bmag
    Jacobi         = geometry.jacobi_matrix(bhat, dxyz).detach().cpu().numpy()
    ...
    raise ValueError(f"`coords`:{coords} is not 'spherical' or 'cartesian'")
print('!!! Code Ending !!!')